Drupal.behaviors.pl_port = {
	attach: function (context, settings) {
		...
		// Intro Gallery
		jQuery('.front .gallery-full.slideshow-on .item-list ul li#image-0').delay(2200).fadeIn(350, function(){
			start_show = setInterval(function(){nextItem();}, 3000);
		});
		
		// Slideshow
		jQuery('.not-front .gallery-full.slideshow-on .item-list ul li#image-0').fadeIn(350, function(){
			start_show = setInterval(function(){nextItem();}, 3000);
		});
		
		// No Slideshow
		jQuery('.not-front .gallery-full.slideshow-off .item-list ul li#image-0').fadeIn(350);
		...
		function selectItem(item_num){
			...
			if(last_item != current_item){
				...
				jQuery('#image-' + current_item).fadeIn(350);
		...
		function thumbClose(){
			jQuery('.gallery-full .images').fadeIn(350);
			jQuery('.gallery-full .thumbs').fadeOut(350);
		}
		
		function thumbOpen(){
			jQuery('.gallery-full .images').fadeOut(350);
			jQuery('.gallery-full .thumbs').fadeIn(350);
		}
		...
		function fadeItems(){
			...
			if(last_item != current_item){
				jQuery('#image-' + current_item).fadeIn(350);
				jQuery('#image-' + last_item).fadeOut(350);
			}
		}
